...
export default function Cart() {
  ...
  const [settings, setSettings] = useState({ gst: 18, deliveryFee: 50 });
  ...
  const fetchSettings = async () => {
    try {
      ...
      setSettings(res.data || { gst: 18, deliveryFee: 50 });
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const fetchCart = async () => {
    try {
      ...
      // 🔥 IMPORTANT FIX: support both formats
      const items =
        res.data?.items ||
        res.data?.cart ||
        [];
  ...
  const gst = subtotal * ((settings?.gst || 0) / 100);
          ...
          <div className={styles.left}>
            ...
            {cart.length === 0 ? (
              ...
              cart.map((item, i) => {
                ...
                return (
                  <div className={styles.lmain} key={item._id || i}>

                    <div className={styles.lcard}>

                      <div className={styles.cimg}>
                        <img src={product.images?.[0]?.url || BULB} alt="" />
                      </div>

                      <div className={styles.cartcen}>
                        <p className={styles.cenhead}>{product.name}</p>
                        <p className={styles.censub}>{product.subName}</p>
                        <p className={styles.cenprice}>₹{product.price}</p>
                      </div>
          ...
          <div className={styles.right}>
            ...
            <div className={styles.rmain}>
              ...
              <div className={styles.rsub}>
                ...
                <p className={styles.value}>₹{subtotal.toFixed(0)}</p>
              </div>

              <div className={styles.rsub}>
                <p>GST ({settings.gst}%)</p>
                <p className={styles.value}>₹{gst.toFixed(0)}</p>
              </div>
            ...
            <div className={styles.rsub}>
              ...
              <p className={styles.value}>₹{total.toFixed(0)}</p>
            </div>
            ...
            <button
              ...
              onClick={() =>
                navigate("/checkout", {
                  state: { cart, subtotal, gst, delivery, total },
                })
              }
            >
              <FaLock /> Go to Checkout
            </button>
